...
document.getElementById('sellButton').addEventListener('click', async () => {
    ...
    const Expo2 = parseFloat(document.getElementById('expenditureInput').value); // Parse input value to float
    const sellData = {
        itemName,
        rate,
        quantity,
    ...
    try {
        // Update user's balance and Expo2 in the database
        const usersRef = ref(database, 'users');
        ...
        userSnapshot.forEach((childSnapshot) => {
            ...
            if (userData.email === email) {
                ...
                // Calculate the total of Expo2 and user balance
                const totalBalanceExpo = currentBalance + currentExpo;

                // Check if the total of Expo2 and user balance is less than the seller data's loss value
                if (totalBalanceExpo < parseFloat(sellData.loss)) {
                    alert("Total of Expo2 and user balance is insufficient. Purchase cannot be completed.");
                ...
                push(ref(database, 'sells'), {
                    ...
                    // Update balance and Expo2 by subtracting values
                    const newBalance = currentBalance - parseFloat(loss); // Subtract loss value
                    const newExpo2 = parseFloat(userData.Expo2) + parseFloat(profit) + parseFloat(loss);
                    
                    // Update user's node with updated balance and Expo2
                    update(ref(database, `users/${childSnapshot.key}`), {
                        Balance: newBalance,
                        Expo2: newExpo2
                    }).then(() => {
                        console.log("User balance and Expo2 updated successfully.");
                        // Update welcome message with the new balance and Expo2
